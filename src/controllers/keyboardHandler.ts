...
export const handleKeyPressed = (code: string, shiftKey: boolean) => {
    ...
    if (code === "KeyB" || code === "KeyS") {
        ...
        Handler.numberKeyPressed(symbol, code);
        ...
        // move stop orders
        Handler.adjustAllExits(symbol);
    } else if (code === 'KeyG') {
        if (shiftKey) {
            Handler.keyGPressedWithShift(symbol);
        } else {
            Handler.keyGPressed(symbol);
        }
    } else if (code === 'KeyW') {
        Handler.swapPositionKeyPressed(symbol);
    } else if (code === 'KeyV') {
        Handler.twoWayBreakout(symbol);
    } else if (code === 'KeyA') {
        Handler.reloadPartialPressed(symbol, shiftKey);
    } else if (code === 'KeyR') {
        Handler.raiseTargetsIfWasLess(symbol);
    } else if (code === 'KeyE') {
        ...
        Handler.replaceWithProfitTakingExitOrders(symbol);
    } else if (code === 'KeyU') {
        AutoTrader.pauseAlgo(symbol);
    } else if (code == 'KeyO') {
        Handler.trailStop(symbol, 5);
    } else if (code == 'KeyI') {
        Handler.trailStop(symbol, 15);
    }
    else {
        codeIsUsed = false;
    }
    ...
    if (codeIsUsed) {
        ...
        Firestore.logInfo(`${code} pressed ${hasShiftKey}`);
        UI.syncAndUpdate(1);
    }
};

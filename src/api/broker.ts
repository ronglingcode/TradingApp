...
export const UpdateAccountUIWithDelay = () => {
    let now = new Date();
    if (now > lastAccountSyncTime) {
        lastAccountSyncTime = new Date(now.getTime() + 1000);
...
export const instantOutOneExitPair = async (
...
*/
export const generateExecutionScript = () => {
    let text = '';
    let oes = Models.getAllOrderExecutions(undefined);
    let agg = aggregateExecutionsPerMinutePerSide(oes);
